...
function Get-LatestReport([string]$pattern) {
  Get-ChildItem .\reports $pattern -ErrorAction SilentlyContinue |
...
$indexData | Group-Object index_code | ForEach-Object {
  if ($_.Group.Count -lt 2) { return }
  $rows = $_.Group | Sort-Object { [datetime]$_.date }
...
# ----------------------------
# 4) Step 7-4 Dashboard (single allocation based on trend)
# ----------------------------
...
# ----------------------------
# 5) Step 7-5 Scenarios (Conservative/Balanced/Aggressive)
# ----------------------------
$scenarioRatios = @()
