...
# ✅ 防止你 Profile 的 StrictMode 影響此工具
Set-StrictMode -Off
$ErrorActionPreference = "Stop"

# ✅ 永遠以「run.ps1 所在資料夾」當作根目錄（修掉跑去 C:\Users\Administrator 的問題）
...
function Write-Utf8NoBom([string]$path, [string]$text) {
  ...
  if (!(Test-Path $dir)) { New-Item -ItemType Directory -Path $dir | Out-Null } # ✅ 再保險一次
...
# ----------------------------
# Paths (absolute via $Root)
# ----------------------------
$basePath   = Join-Path $Root "data"
...
$sectorCount = @($sectorHeat).Count

if ($sectorCount -le 5) {
  # ✅ 類股太少：Bottom 直接顯示「弱→強」全部
  $bot = @($sectorHeat | Sort-Object score)
} else {
  # ✅ 類股足夠：維持 Bottom 不跟 Top 重疊
  $botAll = @($sectorHeat | Sort-Object score)
  $bot = @()
  foreach ($s in $botAll) {
    if (@($top | Where-Object { param(
  [Parameter(Mandatory=$false)]
  [int]$Cash = 300000,
