...
param(
  [string]$Date = "",
  [int]$Capital = 300000,
  [Alias('MaxPositions')][int]$Top = 4000,
...
# --- compat: MaxPositions alias to Top ---
if($PSBoundParameters.ContainsKey("MaxPositions") -and $MaxPositions -gt 0){
  try { $Top = [int]$MaxPositions } catch {}
}
...
# ---- STEP A: pick symbols (prefer allocation_log; else decisions) ----
$symbols = @()
if (Test-Path $allocLogPath) {
  try { $symbols = @(Import-Csv $allocLogPath | Select-Object -ExpandProperty code | Where-Object { $_ -match '^\d{4}$' } | Sort-Object -Unique) } catch { $symbols = @() }
}
...
if ($symbols.Length -eq 0) { throw "No symbols available. Need allocation_log.csv or decisions.csv to derive symbols." }
...
& $perfReport -HoldingDays $HoldingDays -AllowPartial -Open:$Open | Out-Host
...
# IMPORTANT: use explicit named params (avoid array splat mis-binding)
& $portReport `
  -UseAllocationWeights:$UseAllocationWeights `
