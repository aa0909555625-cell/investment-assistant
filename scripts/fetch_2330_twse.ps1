...
# 安裝 requests/pandas（用 python -m pip，避免 pip 指到別的環境）
python -m pip install -q --upgrade pip
python -m pip install -q pandas requests

# 產生 python 抓取程式（TWSE 月資料 → 合併 → 轉成 OHLCV）
@"
import sys
...
"@ | Set-Content "scripts\fetch_twse_2330.py" -Encoding UTF8

python "scripts\fetch_twse_2330.py" $StockNo $MonthsBack
