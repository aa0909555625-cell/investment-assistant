...
# ---- Latest reports (quick links) ----
function Get-LatestReport([string]$filter){
  try {
    $f = Get-ChildItem $reportsDir -Filter $filter -ErrorAction Stop | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($f) { return $f.FullName }
  } catch { }
  return ""
}
$latestPerfHtml  = Get-LatestReport "performance_report_*.html"
$latestEquityHtml = Get-LatestReport "equity_curve_*.html"
$latestPortHtml  = Get-LatestReport "portfolio_curve_*.html"

function LinkOrNA([string]$path, [string]$label){
  if ([string]::IsNullOrWhiteSpace($path) -or !(Test-Path $path)) { return "<span class='small'>N/A</span>" }
  $u = ("file:///" + ($path -replace "\\","/"))
  return ("<a href='{0}' style='color:#66ccff' target='_blank'>{1}</a> <span class='small'>({2})</span>" -f $u, (HtmlEncode $label), (HtmlEncode ([System.IO.Path]::GetFileName($path))))
}

$quickLinksHtml = @(
  "<div class='small'>Performance: " + (LinkOrNA $latestPerfHtml "Open") + "</div>",
  "<div class='small'>Equity Curve: " + (LinkOrNA $latestEquityHtml "Open") + "</div>",
  "<div class='small'>Portfolio Curve: " + (LinkOrNA $latestPortHtml "Open") + "</div>"
) -join "`n"

# ---- Risk Card (portfolio_curve_*.csv) ----
$risk = [ordered]@{
  HasData = $false
  CsvName = ""
  Days = ""
...
try {
  $pc = Get-ChildItem $reportsDir -Filter "portfolio_curve_*.csv" -ErrorAction Stop |
