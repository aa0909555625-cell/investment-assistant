...
# ---- Risk Card: read latest portfolio_curve_*.csv ----
$risk = [ordered]@{
  HasData = $false
  CsvName = ""
  Days = ""
...
try {
  ...
  # ignore risk card errors
}

# ---- Allocation Suggestion (simple, deterministic) ----
# Use Top candidates; allocate equally among score>=50
...
if ($cands.Length -gt 0) {
  ...
  $allocHtml = "<tr><td colspan='4'>No candidates with score >= 50 for allocation.</td></tr>"
}
...
<h2>市場盤口總覽 (Decisions)  $targetDate</h2>
...
<div class="section card">
  <div style="display:flex; justify-content:space-between; align-items:center;">
    <div><b>風控卡片 (Day-level Portfolio Curve)</b></div>
  ...
  <div class="small" style="margin-top:8px;">
    若 RiskOffNext=True：明日建議降曝險（RiskOffScale）。若 MDD 接近門檻：建議降低 ExposureCap 或提高 ScoreThreshold。
  </div>
</div>
