...
$medChg = [math]::Round((($today | Sort-Object {[double]$_.change_percent} | Select-Object -ExpandProperty change_percent)[[int]($today.Length/2)]), 4)

# sector heat (using existing 'sector' field; if unknown -> "unknown")
$bySector = $today | ForEach-Object {
  $sec = $_.sector
...
} | Group-Object sector | ForEach-Object {
...
} | Sort-Object avg_score -Descending, avg_change -Descending | Select-Object -First $SectorTop
